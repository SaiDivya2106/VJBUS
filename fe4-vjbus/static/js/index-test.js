...
function setupSocketEvents() {
    ...
    socket.on("location_update", function(data) {
        ...
        activeRoutes.add(data.route_id);
        
        // Only update the specific route option in the dropdown if it exists
        const routeOption = document.querySelector(`#routeSelect option[value='${data.route_id}']`);
        if (routeOption && !routeOption.textContent.includes("🟢")) {
            routeOption.textContent = `${data.route_id} 🟢`;
        }
    } else if (data.status === "stopped") {
        activeRoutes.delete(data.route_id);
